...
class GestorStock:
    ...
    def __init__(
        ...
        # --------------------------------------------
        # Directorios y rutas de exportación DEMO
        # --------------------------------------------

        # Raíz del proyecto (dos niveles arriba de este archivo)
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

        # Carpeta general de exportaciones
        self.EXPORT_DIR = os.path.join(project_root, "exports")

        # Subcarpetas por tipo
        self.EXPORT_CSV_DIR = os.path.join(self.EXPORT_DIR, "csv")
        self.EXPORT_EXCEL_DIR = os.path.join(self.EXPORT_DIR, "excel")
        self.IMPRIMIR_DIR = os.path.join(self.EXPORT_EXCEL_DIR, "imprimir")

        # Crear directorios si no existen
        for d in (
            self.EXPORT_DIR,
            self.EXPORT_CSV_DIR,
            self.EXPORT_EXCEL_DIR,
            self.IMPRIMIR_DIR,
        ):
            os.makedirs(d, exist_ok=True)

        # --------------------------------------------
        # Ficheros DEMO de importación (Excel mock)
        # --------------------------------------------
        # Ubicados dentro de data_example
        self.ALBARANES_EXCEL = os.path.join(
            project_root, "data_example", "ALBARANES_SERVIDOS_DEMO.xlsx"
        )
        self.PEDIDOS_EXCEL = os.path.join(
            project_root, "data_example", "PEDIDOS_PENDIENTES_DEMO.xlsx"
        )
    ...
    def _export_csv(
        ...
        # ✅ Usamos siempre la carpeta de CSV (exports/csv)
        export_dir = getattr(self, "EXPORT_CSV_DIR", getattr(self, "EXPORT_DIR", "."))
        os.makedirs(export_dir, exist_ok=True)

        ruta = os.path.join(export_dir, f"{nombre_base}_{fecha}.csv")
        try:
            # utf-8-sig para que Excel no se líe con acentos
    ...
    def _exportar_todos_los_datos(self) -> None:
        ...
        ordenes_sorted = sorted(
            ordenes_export, key=lambda x: (x["MODELO"], talla_sort_key(x["TALLA"]))
        )
        ordenes_con_totales = []
        total_general_ord = 0
        total_modelo = 0
        ...
        pedidos_export_sorted = sorted(
            pedidos_export, key=lambda x: (x["MODELO"], talla_sort_key(x["TALLA"]))
        )
        pedidos_con_totales = []
        ...
        low_stock_export = [
            item for item in estimado_export if item["STOCK_ESTIMADO"] < 10
        ]
        self._export_csv(
